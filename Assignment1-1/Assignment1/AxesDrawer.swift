...
{
    var color: UIColor
    var contentScaleFactor: CGFloat             // Setzen vom UIView contentScaleFactor um die Achsen mit maximaler Genauigkeit zu positionieren
    var minimumPointsPerHashmark: CGFloat = 40  // Public, wollen wir normalerweise nicht neu setzen
    ...
    /* Diese Methode ist das Herz des AxesDrawer. Zeichnet im Koordinatenstystem des aktuellen 
       Graphic Context. Deshalb muessen origin und bounds im Koordinatensystem des aktuellen 
       Graphic Context sein. pointPerUnit ist praktisch die "Skalierung" der Achsen. 
       Wenn wir also z.B. 100 Points entlang einer Achse zwischen -1 und 1 haben moechten, 
       dann setzen wir pointsPerUnit auf 50.
    */
    ...
    // Ab hier ist alles private!

    private struct Constants {
        static let hashmarkSize: CGFloat = 6
    }
    
    private let formatter = NumberFormatter() // Formatierung fuer Labels
    ...
    private func drawHashmarks(in rect: CGRect, origin: CGPoint, pointsPerUnit: CGFloat) {
        if ((origin.x >= rect.minX) && (origin.x <= rect.maxX)) || ((origin.y >= rect.minY) && (origin.y <= rect.maxY)) {
            // Wie viele Einheiten pro Hashmark in Abhaenigkeit von pointsPerUnit und minimumPointsPerHashmark
            ...
            // Welches sind die naehsten Hashmarks (vom Urspring) im Rechteck
            ...
            // ... bis die Hashmarks ausserhalb der Boundig Box liegen
